...
func TestEtcdClusterDNSNames(t *testing.T) {
	usEast1 := clusterapi.RegionForName("us-east-1")
	usWest1 := clusterapi.RegionForName("us-west-1")
	etcdNet := NewNetwork([]clusterapi.Subnet{}, []clusterapi.NATGateway{})
	etcdCount := 1

	t.Run("WithENI", func(t *testing.T) {
		t.Run("WithoutCustomDomain", func(t *testing.T) {
			config := clusterapi.EtcdCluster{
