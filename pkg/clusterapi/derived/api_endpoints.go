...
func NewAPIEndpoints(configs []clusterapi.APIEndpoint, allSubnets []clusterapi.Subnet) (APIEndpoints, error) {
	endpoints := map[string]APIEndpoint{}

	findSubnetByReference := func(ref clusterapi.SubnetReference) (*clusterapi.Subnet, error) {
	...
	findSubnetsByReferences := func(refs []clusterapi.SubnetReference) ([]clusterapi.Subnet, error) {
		subnets := []clusterapi.Subnet{}
		for i, r := range refs {
			s, err := findSubnetByReference(r)
			if err != nil {
				return []clusterapi.Subnet{}, fmt.Errorf("error in subnet ref at index %d: %v", i, err)
			}
	...
	for i, c := range configs {
		var err error
		var lbSubnets []clusterapi.Subnet
