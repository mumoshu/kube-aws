...
func (e CachedEncryptor) EncryptedBytes(raw []byte) ([]byte, error) {
	return e.EncryptionService.Encrypt(raw)
}

func (e CachedEncryptor) EncryptedCredentialFromPath(filePath string, defaultValue *string) (*EncryptedFile, error) {
	...
	if err != nil {
		...
		cache, err = EncryptedCredentialCacheFromRawCredential(raw, e.EncryptionService)
		if err != nil {
			return nil, err
		}
