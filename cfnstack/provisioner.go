...
func NewProvisioner(name string, stackTags map[string]string, s3URI string, region clusterapi.Region, stackPolicyBody string, session *session.Session, options ...string) *Provisioner {
	p := &Provisioner{
		stackName:       name,
		stackTags:       stackTags,
...
func (c *Provisioner) uploadAsset(s3Svc S3ObjectPutterService, asset clusterapi.Asset) error {
	bucket := asset.Bucket
	key := asset.Key
